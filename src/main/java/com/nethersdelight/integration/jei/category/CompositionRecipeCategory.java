...
public class CompositionRecipeCategory implements IRecipeCategory<CompositionDummy>
{
	...
	public CompositionRecipeCategory(IGuiHelper helper) {
		...
		richSoulSoil = new ItemStack(NDBlocks.RICH_SOUL_SOIL.get());
		icon = helper.createDrawableIngredient(VanillaTypes.ITEM, richSoulSoil);
	...
	public void setRecipe(IRecipeLayoutBuilder recipeLayout, CompositionDummy compositionRecipe, IFocusGroup iIngredients) {
		...
		recipeLayout.addSlot(RecipeIngredientRole.INPUT, 9, 26).addItemStack(soulCompost);
		recipeLayout.addSlot(RecipeIngredientRole.OUTPUT, 93, 26).addItemStack(richSoulSoil);
	...
	private static TranslatableComponent translateKey(@Nonnull String suffix) {
		return new TranslatableComponent(NethersDelight.MODID + ".jei.composition" + suffix);
	}
}